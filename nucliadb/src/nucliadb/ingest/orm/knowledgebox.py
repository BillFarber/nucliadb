...
RESOURCE_TO_DELETE_STORAGE = (
    f"{RESOURCE_TO_DELETE_STORAGE_BASE}/{{kbid}}/{{uuid}}"
)
...
class KnowledgeBox:
        ...
        *,
        ...
        if not kbid:
            raise KnowledgeBoxCreationError(
                "A kbid must be provided to create a new KB"
            )
        if not slug:
            raise KnowledgeBoxCreationError(
                "A slug must be provided to create a new KB"
            )
        ...
        if len(semantic_models) == 0:
            raise KnowledgeBoxCreationError(
                "KB must define at least one semantic model"
            )
        ...
        try:
            async with driver.rw_transaction() as txn:
                ...
                # Create in maindb
                await datamanagers.kb.set_kbid_for_slug(
                    txn, slug=slug, kbid=kbid
                )
                ...
                for vectorset_id, semantic_model in semantic_models.items():  # type: ignore
                    ...
                    if len(semantic_model.matryoshka_dimensions) > 0:
                        dimension = choose_matryoshka_dimension(
                    ...
                    await datamanagers.vectorsets.set(
                        txn, kbid=kbid, config=vectorset_config
                    )

                stored_external_index_provider = (
                    await cls._maybe_create_external_indexes(
                        kbid,
                        request=external_index_provider,
                        indexes=vs_external_indexes,
                    )
                )
                rollback_ops.append(
                    partial(
            ...
            for op in reversed(rollback_ops):
                try:
                    ...
                    logger.exception(
                        f"Unexpected error rolling back {name}. Keep rolling back"
                    )
            raise exc
    ...
    async def update(
        ...
        exist = await datamanagers.kb.get_config(
            txn, kbid=uuid, for_update=True
        )
        if not exist:
        ...
        if config and exist != config:
            ...
            exist.hidden_resources_hide_on_creation = (
                config.hidden_resources_hide_on_creation
            )

        if (
            exist.hidden_resources_hide_on_creation
            and not exist.hidden_resources_enabled
        ):
            raise KnowledgeBoxCreationError(
                "Cannot hide new resources if the hidden resources feature is disabled"
            )
    ...
    async def delete(cls, driver: Driver, kbid: str):
        async with driver.rw_transaction() as txn:
            ...
            shards_obj = await datamanagers.cluster.get_kb_shards(
                txn, kbid=kbid
            )
        ...
        if shards_obj is None:
            logger.warning(
                f"Shards not found for KB while deleting it",
                extra={"kbid": kbid},
            )
        else:
            ...
            if nidx_api is not None:
                for shard in shards_obj.shards:
                    if shard.nidx_shard_id:
                        await nidx_api.DeleteShard(
                            noderesources_pb2.ShardId(id=shard.nidx_shard_id)
                        )

        if kb_config is not None:
            await cls._maybe_delete_external_indexes(
                kbid, kb_config.external_index_provider
            )
    ...
    async def purge(cls, driver: Driver, kbid: str):
        ...
        async with driver.rw_transaction() as txn:
            ...
            if shards_obj is None:
                logger.warning(
                    f"Shards not found for KB while purging it",
                    extra={"kbid": kbid},
                )
            else:
                for shard in shards_obj.shards:
                    if shard.nidx_shard_id:
                        try:
                            await nidx_api.DeleteShard(
                                noderesources_pb2.ShardId(
                                    id=shard.nidx_shard_id
                                )
                            )
                            logger.debug(
                            ...
                            if exc.code() == StatusCode.NOT_FOUND:
                                continue
                            raise ShardNotFound(
                                f"{exc.details()} @ shard {shard.nidx_shard_id}"
                            )
    ...
    async def get_resource_shard(
        ...
        async with datamanagers.with_ro_transaction() as txn:
            ...
            if pb is None:
                logger.warning(
                    "Shards not found for kbid", extra={"kbid": self.kbid}
                )
                return None
    ...
    async def maindb_delete_resource(self, uuid: str):
        ...
        await self.txn.delete_by_prefix(
            KB_RESOURCE.format(kbid=self.kbid, uuid=uuid)
        )
        if basic and basic.slug:
            try:
                await self.txn.delete(
                    KB_RESOURCE_SLUG.format(kbid=self.kbid, slug=basic.slug)
                )
            except Exception:
    ...
    async def create_vectorset(self, config: knowledgebox_pb2.VectorSetConfig):
        if await datamanagers.vectorsets.exists(
            ...
            raise VectorSetConflict(
                f"Vectorset {config.vectorset_id} already exists"
            )
        ...
        # key kind
        config.storage_key_kind = (
            knowledgebox_pb2.VectorSetConfig.StorageKeyKind.VECTORSET_PREFIX
        )
        await datamanagers.vectorsets.set(
            self.txn, kbid=self.kbid, config=config
        )

        # Remove the async deletion mark if it exists, just in case there was a previous deletion
        deletion_mark_key = KB_VECTORSET_TO_DELETE.format(
            kbid=self.kbid, vectorset=config.vectorset_id
        )
    ...
    async def vectorset_marked_for_deletion(self, vectorset_id: str) -> bool:
        key = KB_VECTORSET_TO_DELETE.format(
            kbid=self.kbid, vectorset=vectorset_id
        )
    ...
    async def delete_vectorset(self, vectorset_id: str):
        ...
        vectorset_count = await datamanagers.vectorsets.count(
            self.txn, kbid=self.kbid
        )
        if vectorset_count == 0:
            raise VectorSetConflict(
                "Deletion of your last vectorset is not allowed"
            )
    ...
    async def _maybe_create_external_indexes(
        ...
        if request.type == ExternalIndexProviderType.PINECONE:
            return await PineconeIndexManager.create_indexes(
                kbid, request, indexes
            )
        elif request.type == ExternalIndexProviderType.MARKLOGIC:
            return await MarkLogicIndexManager.create_indexes(
                kbid, request, indexes
            )
        else:
    ...
    async def _maybe_delete_external_indexes(
        ...
        elif stored.type == ExternalIndexProviderType.MARKLOGIC:
            await MarkLogicIndexManager.delete_indexes(kbid, stored)
...
async def catalog_delete_kb(txn: Transaction, kbid: str):
    ...
    async with txn.connection.cursor() as cur:
        await cur.execute(
            "DELETE FROM catalog where kbid = %(kbid)s", {"kbid": kbid}
        )
